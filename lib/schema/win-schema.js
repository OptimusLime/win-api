...
function winSchema(localConfiguration)
{
  ...
  self.getDatabaseObjects = function(type, widObjects)
  {
    ...
    for(var refPath in typeRefs)
    {
      ...
      var isArray = (splitPathRaw[splitPathRaw.length -1] == "items");
      ...
      for(var wid in traverseObjects)
      {
        ...
        //grab the objects at this path location
        var allObjects = tob.get(pathToObject);
    ...
    //  //save this type for the object -- since we've changed
    //   typeToObjects[type][wid] = tob.clone();
    // }
    ...
    //this will create a collection of types and objects -- we can store them later!
    // var typeToObjects = getAllTypeDependencyArrays(type);

    // //build a list of connections between the objects (for a certain type) and the parents they reference
    // var typeToParentConnections = {};

    // //lets start by grabbing our references for the larger type
    // var typeRefs = self.schemaReferences[type];

    // //these have all the location of the references for the ENTIRE schema object
    // for(var refPath in typeRefs)
    // {
    //   //we check what type of ref we have
    //   var schemaRefInfo = typeRefs[refPath];

    //   //what schema are we at this location??
    //   var schemaType = schemaRefInfo.schemaType;
    ...
    //   //let's pull these bad boys our of our other objects
    //   var pathToObject = self.stripObjectPath(refPath);
    ...
    //   //let's loop through each object and pull the objects and the references
    //   for(var wid in traverseObjects)
    //   {
    //     var tob = traverseObjects[wid];
    ...
    //     //we treat it as if we have many objects -- even if it's only 1 -- makes it more generic
    //     for(var w=0; w < allObjects.length; w++)
    //     {
    //       var refObject = allObjects[w];
    //       //we have an object -- lets add it, replace it in the original
    //       replaceObjects.push({wid: refObject.wid});
    //       dbObjects.push(refObject);
