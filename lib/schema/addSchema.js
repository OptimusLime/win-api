...
function extendAddSchema(self, metaAdditions)
{
  ...
  //meta info is something added to all win objects when saving
  if(metaAdditions)
    defaultWINAdd['meta'] = metaAdditions;
  ...
  //around things if it's not formatted but we would like to make it less wordy to make schema
    self.internalAddSchema = function(type, schemaJSON, options)
    {
      ...
      {
        return {error: "Array-types for schema cannot have WIN additions. It doesn't make any sense. The object must be an array, but also have a wid property? Failed: " + type};
      }
      ...
      {
         return {error: ("Improper schema format for " + type + " - " + JSON.stringify(schemaCheck))};
      }
      ...
      else
      {
        ...
        return {schema: traverse(schemaJSON).clone()};
      }
    }
