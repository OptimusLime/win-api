...
module.exports = function(winShared)
{
	...
	self.syncLoadCustomSchema = function(type, schemaJSON)
	{
		try{
			...
			winShared.schemaManager.addSchemaToManager(type, schemaJSON);
		}
		catch(e)
		{
	...
	self.loadRecentArtifactsByProperties = function(queryProps, startDate, count, type)
	{
		if(!type)
			type = winShared.mainArtifactType;
		...
		//we can now load by calling our schema manager -- this is a simple wrapper
		//in the future, we will handle all the errors coming out with more specificity 
		 winShared.schemaManager.asyncLoadRecentObjectsByProperties(queryProps, startDate, count, type)
			.then(function(args)
			{
				defer.resolve(args);
			}, function(err){
