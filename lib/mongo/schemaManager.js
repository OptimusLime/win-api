...
module.exports = function(schemaConverter, schemaValidator)
{
    ...
    schemaManager.loadAllSchemas = function(paths)
    {
        ...
        for(var type in allSchema)
        {
            schemaManager.addSchemaToManager(type, allSchema[type], undefined, true);
        }
    ...
    schemaManager.addSchemaToManager = function(type, schemaJSON, collectionName, addConnectionType)
    {
        ...
        schemaConverter.schema(type, processedSchema, collectionName);
    ...
    function cleanModels(models, options)
    {
        options = options || {};
        ...
        for(var i=0; i < models.length; i++)
        {
            models[i] = traverse(models[i]).map(function(node)
                {
                    if (options.remapID && node._id){
                        node[options.remapID] = node._id;
                        delete node._id;
                        this.update(node);
                    }
                    else{
    ...
    schemaManager.asyncLoadObjects = function(type, widList, options)
    {
        ...
        MongoModel.find({wid: {"$in": widList}}).lean().exec(function(err, models)
        {
            ...
            defer.resolve(cleanModels(models, options));
        });
    ...
    //keep in mind, we aren't doing any checks on who can access what -- that must come before calling these objects
    schemaManager.asyncCreateOrRemove = function(type, aQueryProps, toSaveObject, options)
    {
        var defer = Q.defer();
        ...
        //we're going to search for this object according to the given properties
        for(var i=0; i < aQueryProps.length; i++){
            var key = aQueryProps[i];
            if(toSaveObject[key] != undefined)
                mongoQuery[key] = toSaveObject[key];
        }

        //find anything matching these wids
        MongoModel.findOneAndRemove(mongoQuery).lean().exec(function(err, model)
        {
            if(err)
            {
                defer.reject(err);
            ...
            {
                 //validate
                var validateObjects = schemaValidator.validateDataArray(type, [toSaveObject]);
                if(validateObjects.isValid)
                {
                    //nothing existed -- lets create and save the model
                    var mongoSaveObject = new MongoModel(toSaveObject);
                    mongoSaveObject.save(function(err)
                    {
                        if(err)
                            defer.reject(err);
                        else
                            defer.resolve({created: true});
                    });
                }
                else
                {
            ...
            {
                //it did exist -- now it's gone -- success!
                defer.resolve({removed: true});
            }
        });
    ...
    schemaManager.asyncLoadRecentObjects = function(startDate, count, type, options)
    {
         //lets fetch these from the appropriate model!  
        var defer = Q.defer();
    ...
    schemaManager.asyncLoadRecentObjectsByProperties = function(queryObject, type, options)
    {
         //lets fetch these from the appropriate model!  
        var defer = Q.defer();
        ...
        var sortObject = options.sort || {date: -1};
        var dateQuery;
        ...
        // console.log('Mod search', dateQuery, 'sort ob', sortObject, ' limi' , options.count, 'model: ', MongoModel);


        //find anything matching these wids
        var search = MongoModel
            .find(dateQuery);

        if(typeof options.sort == "object" && Object.keys(options.sort).length)
            search = search.sort(sortObject);

        if(options.skip)
            search = search.skip(options.skip);

        //absolutely no more than 150 fetched under any weird circumstances -- 10 if not provided
        var count = Math.min(150, options.count || 10);

        search //if nothign provided -- only fetch 10 MAX -- otherwise listen to the incoming object
            .limit(count)
            .lean()
            .exec(function(err, models)
            {
                    ...
                    for(var refType in rawObjects)
                    {
                        ...
                        for(var wid in oObjects)
                        {
                            ...
                            if(trueWids[wid])
                            {
                                ...
                                //tag the date of every reference object being saved please
                                dbReady.date = Date.now();
