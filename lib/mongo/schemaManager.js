...
module.exports = function(schemaConverter, schemaValidator)
{
    ...
    schemaManager.loadAllSchemas = function(paths)
    {
        ...
        for(var type in allSchema)
        {
            ...
            var processedSchema = schemaValidator.addSchema(type, allSchema[type]);

            if(processedSchema.error)
                throw new Error(error);

            //no error? grab the schema 
            processedSchema = processedSchema.schema;
    ...
    schemaManager.asyncValidateAndSaveObjects = function(type, widToObjects)
    {
        ...
        if(validateObjects.isValid)
        {
            ...
            var allRawDB = schemaValidator.getDatabaseObjects(type, widToObjects);
                    ...
                    for(var refType in rawObjects)
                    {
                        ...
                        for(var wid in oObjects)
                        {
                            ...
                            if(trueWids[wid])
                            {
                                ...
                                var dbReady = MongoModel(oObjects[wid]);
        ...
        Q.allSettled(allPromises)
            .then(function(results)
            {
                ...
                for(var i=0; i < results.length; i++)
                {
                    ...
                    {
                        defer.reject({message: "Some wid checks failed to run. Try again."});
                        return;
                    }
                }

                //when we're all done, resolve with the true objects
                defer.resolve(trueWidsToSave);
            })
    ...
    function asyncReturnExisting(type, MongoModel, widList)
    {
            ...
            .lean().exec(function(err, aModels)
        {
            ...
            for(var i=0; i < aModels.length; i++)
                discovered.push(aModels.wid);

            defer.resolve({type: type, existing: discovered});
        });
