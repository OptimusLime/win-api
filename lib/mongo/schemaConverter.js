...
module.exports = function()
{
    ...
    var schema = function (name, descriptor, collectionName) {
    ...
    var mapConvert = function (obj) {
        ...
        // console.log('Traverseed: '.blue, traversed);

        //lets remove properties and arrays from the schema
        var reducedSet = traversed.map(function(node)
        {
            // console.log("Reduce: ".red, this.key, "\n node \n".magenta, this.node);   
            if(this.node.items)
            {
                this.update([this.node.items]);
            }
            else if(this.node.properties)
            {
                this.update(this.node.properties);
            }
        });
    
        // console.log('\t reducedSet '.blue, require('util').inspect(reducedSet,false, 10));

        // console.log("Reduced: ".green, require('util').inspect(reducedSet, false, 10));
        ...
        var removeTypes = traverse(reducedSet).map(function(node)
        {
            if(this.isLeaf)
            {
                // console.log("Trav: ", this.key, " node: ", node);
        ...
        // console.log('\t removedTypes '.blue, require('util').inspect(removeTypes,false, 10));


        // console.log("Replaced: ".green, require('util').inspect(removeTypes, false, 10));

        //Now, inside mongoose, arrays are a bit of a nuisance
        //each array (if it's more than a simple type) must be a schema object
        //the$refore, we must go through and replace any complicated arrays
...
//            console.log(this.key + " leaf? " + this.isLeaf);
        ...
        // console.log('\t schemaArrays '.blue, require('util').inspect(schemaArrays,false, 4));
